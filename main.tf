...
resource "google_spanner_database" "database" {
  ...
  ddl = [
    "CREATE TABLE user_bal_new (UserId INT64 NOT NULL, Balance FLOAT64 NOT NULL, CreationTime TIMESTAMP NOT NULL ) PRIMARY KEY (UserId)",
    "CREATE TABLE user_txns_new (UserId INT64 NOT NULL, TransactionId INT64 NOT NULL, Amount FLOAT64 NOT NULL, CreationTime TIMESTAMP NOT NULL ) PRIMARY KEY (UserId, TransactionId), INTERLEAVE IN PARENT user_bal_new ON DELETE CASCADE",
  ]
}